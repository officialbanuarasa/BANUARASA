...
class StorageService {
  ...
  init() {
    ...
    // IMPORTANT: Google Spreadsheet synchronization is MANUAL ONLY.
    // Do not pull from Apps Script during constructor/init; this avoids
    // overwriting the local session before the user explicitly refreshes.
  ...
  addAnnouncement(ann: Omit<Announcement, 'announcement_id' | 'created_at'>, adminId = 'SUPER_ADMIN'): Announcement {
    ...
    this.logAudit({
      ...
      module: 'ANNOUNCEMENT',
      reference_id: id,
      description: `Super Admin menerbitkan pengumuman baru "${newAnn.title}" (${newAnn.category})`,
      result: 'SUCCESS',
    });

    this.addNotification({
      title: 'Pemberitahuan Baru dari Bara',
  ...
  updateAnnouncement(
    ...
    const anns = this.getAnnouncements();
    const idx = anns.findIndex((a) => a.announcement_id === announcementId);
    if (idx === -1) return false;
    ...
    this.logAudit({
      ...
      description: `Super Admin memperbarui redaksi pengumuman "${anns[idx].title}" (${announcementId})`,
      result: 'SUCCESS',
    });
  ...
  deleteAnnouncement(announcementId: string, adminId = 'SUPER_ADMIN'): boolean {
    const anns = this.getAnnouncements();
    const target = anns.find((a) => a.announcement_id === announcementId);
    if (!target) return false;

    const filtered = anns.filter((a) => a.announcement_id !== announcementId);
    ...
    this.logAudit({
      ...
      module: 'ANNOUNCEMENT',
      reference_id: announcementId,
      description: `Super Admin menghapus pengumuman "${target.title}" (${announcementId})`,
      result: 'SUCCESS',
    });
  ...
  addSponsor(sponsor: Omit<Sponsor, 'sponsor_id'>, adminUsername = 'SUPER_ADMIN'): Sponsor {
    const sponsors = this.getSponsors();
    const id = `SPS-${Date.now().toString().slice(-4)}`;
    ...
    this.logAudit({
      ...
      user_role: 'SUPER_ADMIN',
      action: 'ADD_SPONSOR',
      module: 'PENTAHELIX_PARTNERS',
      reference_id: id,
      description: `Super Admin menambahkan logo mitra perusahaan "${newSponsor.sponsor_name}" (${newSponsor.category || 'Mitra'})`,
      result: 'SUCCESS',
    });
  ...
  updateSponsor(
    ...
    const sponsors = this.getSponsors();
    const idx = sponsors.findIndex((s) => s.sponsor_id === sponsorId);
    if (idx === -1) return false;
    ...
    this.logAudit({
      ...
      description: `Super Admin memperbarui data logo mitra "${sponsors[idx].sponsor_name}"`,
      result: 'SUCCESS',
    });
  ...
  deleteSponsor(sponsorId: string, adminUsername = 'SUPER_ADMIN'): boolean {
    const sponsors = this.getSponsors();
    const target = sponsors.find((s) => s.sponsor_id === sponsorId);
    if (!target) return false;

    const filtered = sponsors.filter((s) => s.sponsor_id !== sponsorId);
    this.setItem(STORAGE_KEYS.SPONSORS, filtered);
    this.notify();
    this.persistToServer();
    ...
    this.logAudit({
      ...
      module: 'PENTAHELIX_PARTNERS',
      reference_id: sponsorId,
      description: `Super Admin menghapus logo mitra "${target.sponsor_name}" (${sponsorId})`,
      result: 'SUCCESS',
    });
  ...
  // --- Member Product Advertisements (Hero Carousel) ---
  getProductAds(): MemberProductAd[] {
    return this.getItem(STORAGE_KEYS.PRODUCT_ADS, INITIAL_PRODUCT_ADS);
  }

  addProductAd(
    ad: Omit<MemberProductAd, 'ad_id' | 'created_at'>,
    adminUsername = 'SUPER_ADMIN'
  ): MemberProductAd {
    const ads = this.getProductAds();
    const id = `AD-${Date.now().toString().slice(-4)}`;
    const newAd: MemberProductAd = {
      ...ad,
      ad_id: id,
      created_at: new Date().toISOString(),
    };
    ads.unshift(newAd);
    this.setItem(STORAGE_KEYS.PRODUCT_ADS, ads);
    this.notify();
    this.persistToServer();
    ...
    this.logAudit({
      user_id: adminUsername,
      user_role: 'SUPER_ADMIN',
      action: 'ADD_PRODUCT_AD',
      module: 'HERO_CAROUSEL',
      reference_id: id,
      description: `Super Admin mengunggah iklan produk anggota "${newAd.title}" (${newAd.store_name})`,
      result: 'SUCCESS',
    });

    this.addNotification({
      title: 'Iklan Produk UMKM Baru',
      message: `Iklan produk ${newAd.product_name} dari ${newAd.store_name} kini tayang di Hero Carousel atas.`,
      type: 'SUCCESS',
    });
  ...
  updateProductAd(
    adId: string,
    updates: Partial<MemberProductAd>,
    adminUsername = 'SUPER_ADMIN'
  ): boolean {
    const ads = this.getProductAds();
    const idx = ads.findIndex((a) => a.ad_id === adId);
    if (idx === -1) return false;
    ...
    this.logAudit({
      user_id: adminUsername,
      user_role: 'SUPER_ADMIN',
      action: 'UPDATE_PRODUCT_AD',
      module: 'HERO_CAROUSEL',
      reference_id: adId,
      description: `Super Admin memperbarui iklan produk "${ads[idx].title}" (${adId})`,
      result: 'SUCCESS',
    });
  ...
  deleteProductAd(adId: string, adminUsername = 'SUPER_ADMIN'): boolean {
    const ads = this.getProductAds();
    const target = ads.find((a) => a.ad_id === adId);
    if (!target) return false;

    const filtered = ads.filter((a) => a.ad_id !== adId);
    this.setItem(STORAGE_KEYS.PRODUCT_ADS, filtered);
    this.notify();
    this.persistToServer();
    ...
    this.logAudit({
      user_id: adminUsername,
      user_role: 'SUPER_ADMIN',
      action: 'DELETE_PRODUCT_AD',
      module: 'HERO_CAROUSEL',
      reference_id: adId,
      description: `Super Admin menghapus iklan produk "${target.title}" (${adId})`,
      result: 'SUCCESS',
    });
  ...
  async updateMemberCardDesign(
    updates: Partial<MemberCardDesignConfig>,
    adminUsername = 'SUPER_ADMIN'
  ): Promise<MemberCardDesignConfig> {
    ...
    // Spreadsheet is the source of truth. Do not report success or update
    // the local cache until Apps Script confirms the design was persisted.
    const response = await callGoogleAppsScript('saveMemberCardDesign', {
      design: updated,
      admin_id: adminUsername,
    });
    const remote = (response as any)?.result ?? (response as any)?.data ?? response;
    if (!response?.success || String(remote?.status || '').toUpperCase() !== 'SAVED') {
      throw new Error(
        remote?.message || response?.error || response?.message ||
        'Desain KTA gagal disimpan ke Google Spreadsheet.'
      );
    }
    ...
    this.addNotification({
      title: 'Desain KTA Digital Disimpan',
      message: 'Desain KTA berhasil disimpan ke Google Spreadsheet dan siap digunakan anggota.',
      type: 'SUCCESS',
    });
  ...
  /**
   * Ambil desain KTA terbaru langsung dari Google Spreadsheet.
   * Dipakai ketika KTA anggota dibuka agar tidak bergantung pada cache lama.
   */
  async refreshMemberCardDesignFromSpreadsheet(): Promise<MemberCardDesignConfig> {
    const response = await callGoogleAppsScript('getMemberCardDesign', {});
    const remote = (response as any)?.result ?? (response as any)?.data ?? response;
    if (!response?.success || String(remote?.status || '').toUpperCase() !== 'FOUND' || !remote?.design) {
      throw new Error(
        remote?.message || response?.error || 'Desain KTA belum tersedia di Google Spreadsheet.'
      );
    }
    const design = { ...DEFAULT_MEMBER_CARD_DESIGN, ...remote.design };
    this.setItem(STORAGE_KEYS.CARD_DESIGN, design);
    this.notify();
    return design;
  }

  /**
   * Simpan profil anggota secara write-through dan hanya memperbarui cache
   * setelah Apps Script mengonfirmasi keberhasilan penyimpanan.
   */
  async saveMemberAndWait(member: Member): Promise<Member> {
    ...
    const response = await googleWorkspaceSync.syncRowToSpreadsheet(
      'SHEET_ANGGOTA_KOPERASI',
      normalized.member_id,
      normalized
    );
    if (!response?.success) {
      throw new Error(response?.error || response?.message || 'Data anggota gagal disimpan ke Google Spreadsheet.');
    }
    const members = this.getMembers();
    const index = members.findIndex((m) => m.member_id === normalized.member_id);
