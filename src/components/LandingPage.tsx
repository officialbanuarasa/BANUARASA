...
import { HeroProductAdCarousel } from './HeroProductAdCarousel';
...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  return (
    <div className="space-y-10 pb-16">
      {/* 0. HERO CAROUSEL FOR MEMBER PRODUCT ADS (AUTOPLAY LEFT-TO-RIGHT) */}
      <HeroProductAdCarousel
        onSelectStand={(code) => {
          scrollToStandDisplay();
          const cat = getStandCategory(code);
          if (cat === 'KATEGORI_1') setActiveStandFilter('VIP');
          else if (cat === 'KATEGORI_2') setActiveStandFilter('KAT2');
          else if (cat === 'KATEGORI_3') setActiveStandFilter('KAT3');
        }}
        isSuperAdmin={currentUser?.role === 'SUPER_ADMIN'}
        onOpenAdminManage={() => {
          setCarouselsInitialTab('PRODUCT_ADS');
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="space-y-2 py-3 border-y border-slate-100 text-xs text-slate-600 font-medium">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>64 Stand Terpadu (VIP, Reguler, Subsidi)</span>
              </div>
            </div>
      ...
      {/* 2. MENU FITUR & PORTAL LAYANAN UTAMA BANUARASA */}
      <section id="menu-fitur-utama-aplikasi" className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-slate-100">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="px-3 py-1 rounded-full bg-emerald-600 text-white font-black text-xs tracking-wide shadow-xs flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                Menu Fitur Aplikasi
              </span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
                Pusat Layanan & Fitur Banuarasa
              </h3>
            </div>
            <p className="text-xs text-slate-500">
              Pilih menu layanan di bawah ini untuk mengakses fasilitas pasar, keanggotaan koperasi, dan manajemen sesuai tingkat akses akun Anda.
            </p>
          </div>

          {/* Current Role Status Indicator */}
          <div className="flex items-center gap-2.5 shrink-0 flex-wrap">
            <div className="px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-2xl flex items-center gap-2">
              <span className={`w-2.5 h-2.5 rounded-full ${
                currentUser?.role === 'SUPER_ADMIN' ? 'bg-purple-600 animate-pulse' :
                currentUser?.role === 'ADMIN_KOPERASI' || currentUser?.role === 'ADMIN_EVENT' ? 'bg-indigo-600 animate-pulse' :
                currentUser?.role === 'MEMBER' ? 'bg-emerald-600 animate-pulse' : 'bg-slate-400'
              }`} />
              <div className="text-left">
                <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wider">Tingkat Akses</p>
                <p className="text-xs font-black text-slate-800">
                  {currentUser ? (
                    currentUser.role === 'SUPER_ADMIN' ? 'Super Admin' :
                    currentUser.role === 'ADMIN_KOPERASI' ? 'Admin Koperasi' :
                    currentUser.role === 'ADMIN_EVENT' ? 'Admin Event' :
                    currentUser.role === 'MEMBER' ? 'Anggota UMKM' : 'Tamu / Publik'
                  ) : 'Tamu / Publik'}
                </p>
              </div>
            </div>
            ...
            {!currentUser ? (
              ...
              <button
                onClick={() => onNavigateTab && onNavigateTab('admin-dashboard')}
                className="px-3.5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer"
              >
                <ShieldCheck className="w-3.5 h-3.5" />
        ...
        {/* 12 Icon Menu Cards Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3.5 sm:gap-4">
          {/* 1. Denah & Pemesanan 64 Stand */}
          <button
            type="button"
            onClick={() => {
              if (onOpenStandMap) {
            ...
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-emerald-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-700 group-hover:bg-emerald-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <Store className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200 truncate">
                Semua User
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-emerald-700 transition-colors leading-snug">
                Denah & Pemesanan Stand
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Tata letak 64 stand pasar mingguan, spesifikasi fasilitas VIP, Reguler, dan Subsidi.
              </p>
            </div>
          ...
          <button
            type="button"
            onClick={() => {
              if (currentUser && (currentUser.role === 'MEMBER' || currentMember)) {
                if (onNavigateTab) onNavigateTab('member-dashboard');
              } else {
                onOpenAuthModal('MEMBER_LOGIN');
              }
            }}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-blue-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-blue-100 text-blue-700 group-hover:bg-blue-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <LayoutDashboard className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-800 border border-blue-200 truncate">
                Anggota UMKM
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-blue-700 transition-colors leading-snug">
                Dashboard Anggota UMKM
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Profil usaha, legalitas NIB/Halal, status KTA, dan riwayat reservasi stand pasar.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-blue-700">
              <span>{currentUser?.role === 'MEMBER' ? 'Masuk Dashboard' : 'Login Anggota'}</span>
              <ChevronRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </div>
          </button>

          {/* 3. KTA Digital & QR Code */}
          <button
            type="button"
            onClick={() => {
              if (currentUser && (currentUser.role === 'MEMBER' || currentMember)) {
                if (onOpenMemberCard) onOpenMemberCard();
                else if (onNavigateTab) onNavigateTab('member-dashboard');
              } else {
                onOpenAuthModal('MEMBER_LOGIN');
              }
            }}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-amber-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-amber-100 text-amber-800 group-hover:bg-amber-500 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <CreditCard className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-900 border border-amber-200 truncate">
                Anggota UMKM
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-amber-800 transition-colors leading-snug">
                KTA Digital & Barcode
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Kartu Tanda Anggota resmi Koperasi Berau Melangkah Bersama ber-QR Code validasi.
              </p>
            </div>
          ...
          <button
            ...
            onClick={() => {
              if (onOpenNoticeBoard) {
                ...
                openEditorial('sejarah');
              }
            }}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-rose-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-700 group-hover:bg-rose-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <Megaphone className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 text-rose-800 border border-rose-200 truncate">
                Semua User
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-rose-700 transition-colors leading-snug">
                Papan Pengumuman Bara
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Warta resmi, jadwal event pasar mingguan, agenda pembinaan, dan siaran pengurus.
              </p>
            </div>
          ...
          <button
            ...
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-teal-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-teal-100 text-teal-700 group-hover:bg-teal-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <DollarSign className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-teal-50 text-teal-800 border border-teal-200 truncate">
                Anggota UMKM
              </span>
            </div>
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Catatan Simpanan Pokok (Rp100rb), Wajib (Rp25rb/bln), dan Sukarela yang transparan.
              </p>
            </div>
          ...
          <button
            ...
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-cyan-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-cyan-100 text-cyan-700 group-hover:bg-cyan-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <TrendingUp className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-cyan-50 text-cyan-800 border border-cyan-200 truncate">
                Anggota UMKM
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-cyan-700 transition-colors leading-snug">
                Laporan Omzet UMKM
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Input catatan omzet harian pelaku usaha untuk evaluasi perputaran pasar mingguan.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-cyan-700">
              <span>Input Omzet</span>
              <ChevronRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </div>
          </button>

          {/* 7. Pindai QR & Barcode Verifikasi */}
          <button
            type="button"
            onClick={() => {
              if (currentUser && (currentUser.role === 'SUPER_ADMIN' || currentUser.role === 'ADMIN_KOPERASI' || currentUser.role === 'ADMIN_EVENT')) {
                if (onOpenQRScanner) onOpenQRScanner();
                else if (onNavigateTab) onNavigateTab('admin-dashboard');
              } else {
                if (onOpenBarcodeModal) onOpenBarcodeModal(currentMember);
                else onOpenAuthModal('MEMBER_LOGIN');
              }
            }}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-purple-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-purple-100 text-purple-700 group-hover:bg-purple-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <QrCode className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-50 text-purple-800 border border-purple-200 truncate">
                Petugas & Anggota
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-purple-700 transition-colors leading-snug">
                Pindai QR & Barcode
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Pemindai kamera verifikasi barcode KTA, tiket stand pasar, dan absensi anggota.
              </p>
            </div>
          ...
          {/* 8. Katalog Produk UMKM */}
          <button
            type="button"
            onClick={() => {
              const el = document.getElementById('katalog-produk-umkm');
              if (el) {
                el.scrollIntoView({ behavior: 'smooth' });
              }
            }}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-orange-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-orange-100 text-orange-700 group-hover:bg-orange-500 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <ShoppingBag className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-orange-50 text-orange-800 border border-orange-200 truncate">
                Semua User
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-orange-700 transition-colors leading-snug">
                Katalog Produk UMKM
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Jelajahi sajian kuliner khas Berau, olahan laut segar, dan kerajinan kriya anggota.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-orange-700">
              <span>Lihat Produk</span>
              <ChevronRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </div>
          </button>

          {/* 9. Google Workspace Hub */}
          <button
            type="button"
            onClick={onOpenGoogleModal}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-emerald-600 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-800 group-hover:bg-emerald-700 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Sinkronisasi database spreadsheet anggota, stand 64, dan dokumen Google Drive.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-emerald-800">
              <span>Inspeksi Cloud</span>
              <ChevronRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </div>
          </button>

          {/* 10. Maskot BARA & Budaya */}
          <button
            type="button"
            onClick={() => {
              if (onOpenSplashIntro) onOpenSplashIntro();
              else openEditorial('maskot-bara');
            }}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-amber-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Kisah Si Kerang Laut, rompi zamrud keemasan, dan filosofi 4 pilar gastronomi pesisir.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-amber-800">
              <span>Kisah Maskot</span>
              <ChevronRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </div>
          </button>

          {/* 11. Pendaftaran Anggota Baru */}
          <button
            type="button"
            onClick={() => onOpenAuthModal('REGISTER')}
            className="group p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white hover:border-emerald-500 hover:shadow-md transition-all text-left flex flex-col justify-between space-y-3 cursor-pointer relative overflow-hidden"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-700 group-hover:bg-emerald-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <UserPlus className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-900 border border-emerald-300 truncate">
                Pendaftaran Buka
              </span>
            </div>
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Formulir pendaftaran pelaku UMKM untuk mendapatkan Nomor Anggota Koperasi resmi.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-emerald-700">
          ...
          {/* 12. Portal Pengurus & Admin */}
          <button
            type="button"
            onClick={() => {
              if (currentUser && (currentUser.role === 'SUPER_ADMIN' || currentUser.role === 'ADMIN_KOPERASI' || currentUser.role === 'ADMIN_EVENT')) {
                if (onNavigateTab) onNavigateTab('admin-dashboard');
              } else {
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="w-12 h-12 rounded-2xl bg-purple-100 text-purple-800 group-hover:bg-purple-900 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-100 text-purple-900 border border-purple-300 truncate">
                Khusus Pengurus
              </span>
            </div>
            <div>
              <h4 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-purple-800 transition-colors leading-snug">
                Portal Pengurus & Admin
              </h4>
              <p className="text-[11px] text-slate-500 mt-1 leading-relaxed line-clamp-2">
                Verifikasi bayar stand, kelola carousel iklan, atur banner mitra, dan audit sistem.
              </p>
            </div>
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-purple-800">
              <span>{currentUser && (currentUser.role === 'SUPER_ADMIN' || currentUser.role === 'ADMIN_KOPERASI' || currentUser.role === 'ADMIN_EVENT') ? 'Buka Admin' : 'Login Admin'}</span>
              <ChevronRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </div>
          </button>
        </div>
      ...
      {/* 7. MITRA STRATEGIS PENTAHELIX — CAROUSEL LOGO PERUSAHAAN / SPONSOR */}
      <PartnerLogoCarousel
        isSuperAdmin={currentUser?.role === 'SUPER_ADMIN'}
        onManagePartners={() => {
          setCarouselsInitialTab('PARTNER_LOGOS');
          setIsCarouselsModalOpen(true);
        }}
      />
      ...
      {/* Superadmin Carousel Content Manager Modal */}
      <AdminCarouselsManagerModal
        isOpen={isCarouselsModalOpen}
        onClose={() => setIsCarouselsModalOpen(false)}
        initialTab={carouselsInitialTab}
        adminUsername={currentUser?.username || 'SUPER_ADMIN'}
      />
    </div>
  );
};
