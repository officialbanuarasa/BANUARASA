    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="bg-slate-50 border-b border-slate-200 px-6 sm:px-8 py-3.5 flex flex-wrap items-center justify-between gap-3 text-xs shrink-0">
          {/* Zone Filter Dropdown Menu (Mobile/Tablet Friendly) */}
          <div className="flex items-center gap-2">
            <span className="text-slate-500 font-bold hidden sm:inline">Kategori Stand:</span>
            <div className="relative inline-flex items-center">
              <span className="absolute left-3 text-emerald-700 pointer-events-none">
                <Filter className="w-3.5 h-3.5" />
              </span>
              <select
                id="modal-zone-filter-select"
                value={activeZoneFilter}
                onChange={(e) => setActiveZoneFilter(e.target.value as any)}
                className="appearance-none pl-8 pr-8 py-1.5 bg-white border border-slate-300 hover:border-emerald-500 rounded-xl text-xs font-bold text-slate-800 shadow-xs focus:ring-2 focus:ring-emerald-500 focus:outline-hidden cursor-pointer"
              >
                <option value="ALL">Semua Stand (64 Stand)</option>
                <option value="A-J">Kategori 1: Stand VIP (A - J) • Rp50.000</option>
                <option value="1-43">Kategori 2: Stand Reguler (1 - 43) • Rp50.000</option>
                <option value="44-54">Kategori 3: Stand Subsidi (44 - 54) • Rp35.000</option>
              </select>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            {currentMember ? (
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs shadow-lg shadow-emerald-500/20 transition-all flex items-center justify-center gap-1.5 w-full sm:w-auto cursor-pointer"
              >
                <span>Masuk / Daftar untuk Memesan Stand {selectedStand.stand_code}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            ) : (
              <button
                type="button"
                onClick={onClose}
                className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs transition-all w-full sm:w-auto cursor-pointer"
              >
