    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 lg:px-8 py-2.5 transition-all shadow-xs">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
        ...
        <nav className="flex items-center gap-1.5 sm:gap-2 text-xs font-bold text-slate-600">
          ...
          {/* Admin Dashboard Link (Only when logged in as Admin / Super Admin) */}
          {currentUser && isAdmin && (
            <button
        ...
        <div className="flex items-center gap-2 shrink-0">
          {/* Papan Pemberitahuan Resmi Bara Button */}
          {onOpenNoticeBoard && (
            <button
              id="btn-navbar-bara-notice"
              onClick={onOpenNoticeBoard}
              className="p-1 sm:px-3 sm:py-1.5 bg-gradient-to-r from-slate-900 to-emerald-950 hover:from-black hover:to-emerald-900 text-white rounded-xl border border-amber-400 shadow-2xs flex items-center gap-2 cursor-pointer transition-all hover:scale-105 active:scale-95"
              title="Buka Papan Pemberitahuan Resmi Bara"
            >
              <div className="w-6 h-6 rounded-lg overflow-hidden border border-amber-400 shrink-0 bg-slate-900">
                <img src={BARA_ASSETS.mascot} alt="Bara" className="w-full h-full object-cover" />
              </div>
              <span className="hidden sm:inline text-xs font-black text-amber-300">
                Papan Bara
              </span>
                    ...
                    <div className="space-y-1 pt-1">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="w-full py-2 px-3 hover:bg-purple-50 text-purple-800 font-bold text-xs rounded-xl flex items-center gap-2 transition-colors cursor-pointer"
                        >
                          <ShieldCheck className="w-3.5 h-3.5 text-purple-600" />
                      ...
                      {onOpenNoticeBoard && (
                        <button
                          ...
                          className="w-full py-2 px-3 hover:bg-amber-50 text-amber-950 font-bold text-xs rounded-xl flex items-center gap-2 transition-colors cursor-pointer"
                        >
                          <div className="w-4 h-4 rounded-full overflow-hidden border border-amber-400 shrink-0">
                            <img src={BARA_ASSETS.mascot} alt="Bara" className="w-full h-full object-cover" />
                          </div>
                          <span>Papan Pemberitahuan Bara</span>
                        </button>
                      )}
